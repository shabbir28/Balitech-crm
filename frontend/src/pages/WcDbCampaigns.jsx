...
const WcDbCampaigns = () => {
    ...
    if (loading) return <div className="flex items-center justify-center h-64 text-slate-400">Loading WC DB Campaigns...</div>;
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-[#1e1e2d] border border-white/10 rounded-2xl w-full max-w-md relative z-10 overflow-hidden shadow-2xl">
                        ...
                        <div className="bg-black/20 p-4 border-t border-white/5 flex justify-end gap-3">
                            <button disabled={deleteModal.isDeleting} onClick={() => setDeleteModal({ isOpen: false, id: null, isDeleting: false })} className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-white hover:bg-white/5">Cancel</button>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">WC DB Campaigns</h1>
                    <p className="text-sm text-slate-500 mt-1">Manage and organize your WC DB campaigns efficiently.</p>
                </div>
                <button onClick={() => navigate('/wc-db-campaigns/add')}
                    className="flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white font-semibold rounded-xl shadow-lg transition-all shrink-0">
            ...
            <div className="overflow-x-auto bg-[#1a1d2e] border border-white/[0.05] shadow-2xl rounded-2xl">
                <table className="min-w-full divide-y divide-white/[0.05] text-sm">
                    ...
                    <tbody className="divide-y divide-white/[0.05]">
                        {campaigns.map(c => (
                            <tr key={c.campaign_id} className="hover:bg-white/[0.02] transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right space-x-3">
                                    <button onClick={() => navigate(`/wc-db-campaigns/edit/${c.campaign_id}`)} className="text-cyan-400 hover:text-cyan-300 transition-colors" title="Edit"><Edit className="h-4 w-4 inline" /></button>
                        ...
                        {campaigns.length === 0 && (
                            <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">No WC DB campaigns found. Add one to get started.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
