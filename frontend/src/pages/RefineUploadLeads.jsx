...
const RefineUploadLeads = () => {
    ...
    useEffect(() => {
        ...
        api.get('/refine-campaigns').then(res => {
    ...
    const handleCreateSession = async () => {
        ...
        try {
            ...
            const res = await api.post('/refine-sessions', {
                vendor_id: selectedVendor,
                campaign_type: campaignName
            });
            if (isCompareMode) {
                navigate(`/refine-sessions/${res.data.id}/add-job?mode=compare`);
            } else {
                navigate(`/refine-sessions/${res.data.id}`);
            }
        } catch (err) {
