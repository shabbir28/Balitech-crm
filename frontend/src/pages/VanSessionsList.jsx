...
const VanSessionsList = () => {
    ...
    const fetchSessions = async (pageToFetch = 1) => {
        ...
        try {
            ...
            const res = await api.get(
                `/Van-sessions?page=${pageToFetch}&limit=${limit}&search=${encodeURIComponent(search)}&status=${encodeURIComponent(statusFilter)}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
            );
    ...
    const executeDelete = async () => {
        ...
        try {
            await api.delete(`/Van-sessions/${deleteModal.id}`);
            fetchSessions(page);
            setDeleteModal({ isOpen: false, id: null, isDeleting: false });
        } catch (err) {
                    ...
                    <div className="bg-[#1e1e2d] border border-white/10 rounded-2xl w-full max-w-xl relative z-10 overflow-hidden shadow-2xl">
                        ...
                        <div className="flex items-center justify-between p-5 border-b border-white/8 bg-black/20">
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-xl bg-violet-500/15 border border-violet-500/25 flex items-center justify-center">
                                    {selectedJobStats ? <BarChart3 className="w-4 h-4 text-violet-400" /> : <Files className="w-4 h-4 text-violet-400" />}
                                </div>
                                <div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {selectedJobStats && (
                                    <button onClick={() => setSelectedJobStats(null)} className="text-[12px] text-violet-400 hover:text-violet-300 font-semibold px-3 py-1 rounded-lg bg-violet-500/10 hover:bg-violet-500/20 transition-colors border border-violet-500/20">
                                        â† Back
                                    </button>
                                )}
                                <button
                        ...
                        {!selectedJobStats ? (
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-center gap-3 bg-[#0a0a0f] border border-white/5 rounded-xl px-4 py-3 transition-all group ${
                                                isCompleted ? 'hover:border-violet-500/40 hover:bg-violet-500/5 cursor-pointer' : 'cursor-default opacity-80'
                                            }`}
                                        >
                                            <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 font-bold text-[10px] border ${
                                                isExcel ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-violet-500/10 border-violet-500/20 text-violet-400'
                                            }`}>
                                                {ext}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className={`text-[13px] font-medium truncate transition-colors ${
                                                    isCompleted ? 'text-white group-hover:text-violet-300' : 'text-slate-400'
                                                }`} title={fileName}>
                                                    {fileName}
                                                </p>
                                            ...
                                            {isCompleted ? (
                                                <span className="text-[9px] bg-violet-500/15 border border-violet-500/25 text-violet-400 px-1.5 py-0.5 rounded font-bold shrink-0">STATS â†’</span>
                                            ) : (
                            ...
                            <div className="p-5 grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
                                ...
                                {/* Fresh */}
                                <div className="bg-[#0a0a0f] rounded-xl border border-violet-500/20 p-4 relative overflow-hidden">
                                    <div className="absolute top-0 left-0 w-1 h-full bg-violet-500"></div>
                                    <div className="flex items-center gap-1.5 mb-2 ml-2"><TrendingUp className="w-3 h-3 text-violet-400" /><p className="text-violet-400 text-[10px] uppercase tracking-widest font-bold">Fresh</p></div>
            ...
            <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between mb-8 gap-6 border-b border-white/5 pb-6">
                <div className="shrink-0">
                    ...
                    <p className="text-slate-400 text-sm mt-3 font-medium flex items-center gap-2">
                        ...
                        <span className="inline-flex items-center justify-center bg-violet-500/10 text-violet-300 border border-violet-500/20 px-2.5 py-0.5 rounded-lg text-xs font-bold font-mono">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-start xl:justify-end">
                    ...
                    <div className="relative group w-full sm:w-auto flex-1 sm:flex-none min-w-[140px] max-w-full sm:max-w-[200px]">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-violet-400">
                            <ListFilter className="w-4 h-4" />
                        </div>
                        <select
                            value={statusFilter}
                            onChange={(e) => setStatusFilter(e.target.value)}
                            className={`bg-[#13151f] border hover:border-violet-500/50 rounded-xl py-2 pl-10 pr-10 outline-none cursor-pointer transition-all w-full text-[13px] font-medium appearance-none shadow-inner focus:ring-2 focus:ring-violet-500/30 ${statusFilter !== 'All' ? 'text-violet-400 border-violet-500/40 bg-violet-500/5' : 'text-slate-300 border-white/10'}`}
                        >
                            {['All', 'Pending', 'Processing', 'Completed', 'Failed'].map((s) => (
                                <option key={s} value={s} className="bg-[#1e1e2d] text-white">{s}</option>
                            ))}
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-slate-500 group-hover:text-violet-400">
                    ...
                    <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full sm:w-auto flex-1">
                        <div className="flex items-center bg-[#13151f] border border-white/10 hover:border-violet-500/50 rounded-xl px-4 py-2 w-full lg:w-64 transition-all focus-within:ring-2 focus-within:ring-violet-500/30 focus-within:border-violet-500 group shadow-inner">
                            <Search className="w-4 h-4 text-slate-500 group-focus-within:text-violet-400 transition-colors shrink-0" />
                            <input
                                type="text"
                        ...
                        <button
                            ...
                            className="bg-violet-500 hover:bg-violet-400 text-white px-6 py-2 rounded-xl font-bold transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)] active:scale-95 text-[13px] w-full sm:w-auto shrink-0"
                        >
                            Search
                        </button>
                    </div>
                ...
                <div className="w-full overflow-x-auto custom-scrollbar rounded-3xl">
                    <div className="min-w-[1500px]">
                        ...
                        <div className="divide-y divide-white/5">
                            {loading ? (
                                <div className="p-20 text-center flex flex-col items-center justify-center gap-4 text-violet-400">
                                ...
                                sessions.map((s) => {
                                    ...
                                    return (
                                        <div key={s.id} className="grid grid-cols-[160px_100px_160px_160px_200px_140px_150px_150px_130px_180px_170px] p-3 items-center hover:bg-white/[0.02] transition-colors group">
                                            ...
                                            {/* Vendor */}
                                            <div className="text-white font-semibold text-[13px] pr-2 group-hover:text-violet-300 transition-colors line-clamp-1">
                                                {s.vendor_name || 'â€”'}
                                            </div>
                                                ...
                                                <div className="w-full h-1.5 bg-black/40 rounded-full overflow-hidden border border-white/5">
                                                    <div 
                                                        className={`h-full rounded-full transition-all duration-700 ease-in-out ${
                                                            ...
                                                            'bg-violet-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]'
                                                        }`}
...
export default VanSessionsList;
