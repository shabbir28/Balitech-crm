...
const PremiumSessionsList = () => {
    ...
    const fetchSessions = async (pageToFetch = 1) => {
        ...
        try {
            ...
            const res = await api.get(
                `/premium-sessions?page=${pageToFetch}&limit=${limit}&search=${encodeURIComponent(search)}&status=${encodeURIComponent(statusFilter)}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
            );
    ...
    const executeDelete = async () => {
        ...
        try {
            await api.delete(`/premium-sessions/${deleteModal.id}`);
            fetchSessions(page);
            setDeleteModal({ isOpen: false, id: null, isDeleting: false });
        } catch (err) {
...
export default PremiumSessionsList;
