...
const PremiumVendors = () => {
    ...
    const fetchVendors = async () => {
        try {
            const res = await api.get('/premium-vendors?counts=false');
            setVendors(res.data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await api.put(`/premium-vendors/${editingId}`, formData);
                showNotification('Vendor updated successfully');
            } else {
                await api.post('/premium-vendors', formData);
    ...
    const executeDelete = async () => {
        ...
        try {
            await api.delete(`/premium-vendors/${deleteModal.id}`);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <h1 className="text-2xl font-bold text-white tracking-tight">Premium Vendor Management</h1>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div className="relative w-full sm:w-72">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-10 py-2.5 bg-[#13151e] border border-white/10 rounded-xl text-white text-sm outline-none focus:border-amber-500/50 placeholder:text-slate-600"
                        />
                        {localSearch && (
                            <button
                                type="button"
                    ...
                    <button 
                        ...
                        className="btn-primary flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white font-semibold rounded-xl shadow-lg transition-all shrink-0"
                    >
            ...
            {activeSearch && (
                <p className="text-sm text-slate-500">
                    Showing <span className="text-amber-400 font-semibold">{filteredVendors.length}</span> vendor(s) for &quot;{activeSearch}&quot;
                </p>
            )}
