...
import { ArrowLeft, ArrowRight, Check, Database, Building2, Layers, Truck } from 'lucide-react';

const VanUploadLeads = () => {
    ...
    useEffect(() => {
        ...
        api.get('/van-campaigns').then(res => {
    ...
    const handleCreateSession = async () => {
        ...
        try {
            ...
            const res = await api.post('/van-sessions', {
                vendor_id: selectedVendor,
                campaign_type: campaignName
            });
            if (isCompareMode) {
                navigate(`/van-sessions/${res.data.id}/add-job?mode=compare`);
            } else {
                navigate(`/van-sessions/${res.data.id}`);
            }
        } catch (err) {
        ...
        <div className="w-full h-full flex flex-col font-sans">
            ...
            <div className="bg-[#1e1e2d] border border-white/5 rounded-2xl p-6 sm:p-8 mb-8 shadow-sm">
                <div className="w-full max-w-4xl mx-auto relative px-4 sm:px-12">
                    ...
                    {/* Progress Line */}
                    <div className="absolute top-6 left-[15%] h-[2px] bg-violet-500 -z-10 rounded-full transition-all duration-500 ease-out" 
                            ...
                            return (
                                <div key={label} className="flex flex-col items-center w-32 text-center group">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 shadow-sm ${
                                        isActive || isPast 
                                            ? 'bg-gradient-to-br from-violet-500 to-violet-600 text-white shadow-[0_0_20px_rgba(139,92,246,0.3)] border border-violet-400/50 scale-110' 
                ...
                {step === 1 && (
                    <div className="w-full bg-[#1e1e2d] border border-white/5 rounded-2xl p-8 sm:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.12)] animate-fade-in relative overflow-hidden">
                        ...
                        <div className="text-center mb-10 relative z-10">
                            <div className="w-14 h-14 bg-violet-500/10 border border-violet-500/20 rounded-2xl flex items-center justify-center mx-auto mb-5 text-violet-400">
                                <Truck size={24} />
                            </div>
                            <h2 className="text-2xl font-bold text-white mb-2 tracking-tight">Select Vendor Source</h2>
                            <p className="text-[14px] text-slate-400 font-medium">Choose the vendor this van data was acquired from to begin the upload session.</p>
                        </div>
                        ...
                        <div className="w-full relative group mb-8 z-10 max-w-lg mx-auto">
                            ...
                            <div className="relative">
                                {/* Decorative Icon */}
                                <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-violet-400">
                                    <Truck className="w-5 h-5" />
                                </div>
                                <select 
                                    className="w-full bg-[#0a0a0f] border-2 border-white/10 hover:border-violet-500/50 text-white rounded-2xl py-4 pl-12 pr-10 appearance-none focus:outline-none focus:ring-4 focus:ring-violet-500/20 focus:border-violet-500 transition-all font-medium shadow-inner cursor-pointer text-[15px]"
                                ...
                                {/* Custom Chevron */}
                                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-5 text-slate-400 group-hover:text-violet-400 transition-colors">
                        ...
                        <div className="w-full flex justify-end mt-4 relative z-10 max-w-lg mx-auto">
                            <button 
                                ...
                                className="bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-500 hover:to-violet-400 text-white px-8 py-3.5 rounded-xl font-semibold transition-all shadow-[0_4px_14px_rgba(139,92,246,0.3)] flex items-center gap-2 active:scale-[0.98] text-[14px]"
                            >
                    ...
                    <div className="w-full bg-[#1e1e2d] border border-white/5 rounded-2xl p-8 sm:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.12)] animate-fade-in relative overflow-hidden">
                        ...
                        <div className="w-full flex justify-between items-center mt-6 relative z-10 max-w-lg mx-auto gap-4">
                            ...
                            <button 
                                ...
                                disabled={creating}
                                className={`flex-1 justify-center text-white px-6 py-3.5 rounded-xl font-semibold transition-all shadow-[0_4px_14px_rgba(139,92,246,0.3)] flex items-center gap-2 active:scale-[0.98] text-[14px] ${
                                    creating 
                                        ? 'bg-violet-500/50 cursor-not-allowed border border-violet-500/20' 
