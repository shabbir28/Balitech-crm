...
import { ArrowLeft, UploadCloud, X, FileText, File } from 'lucide-react';

const VanAddCampaign = ({ editMode = false }) => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchCampaign = async () => {
            try {
                const res = await api.get(`/van-campaigns/${id}`);
                const data = res.data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editMode) {
                await api.put(`/van-campaigns/${id}`, payload);
            } else {
                await api.post('/van-campaigns', payload);
            }

            navigate('/van-campaigns');
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="min-h-screen font-sans" style={{ background: '#323644', margin: '-2rem', padding: '2rem' }}>
            ...
            <div className="flex items-center space-x-4 mb-8 text-white">
                <button 
                    onClick={() => navigate('/van-campaigns')}
                    className="w-10 h-10 bg-[#8b5cf6] rounded-full flex items-center justify-center hover:bg-violet-400 transition-colors shadow-md"
                >
                ...
                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col md:flex-row gap-6 items-stretch">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center">
                                    ...
                                    <input 
                                        ...
                                        className="flex-1 bg-[#4E515E] border-none rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:ring-1 focus:ring-violet-500 outline-none transition-all text-sm shadow-inner"
                                    />
                                </div>
                                ...
                                <div className="flex items-center">
                                    ...
                                    <div className="flex-1 relative">
                                        <input 
                                            ...
                                            onChange={handleChange}
                                            type="date" 
                                            className="w-full bg-[#4E515E] border-none rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-violet-500 outline-none transition-all text-sm shadow-inner [color-scheme:dark]"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start">
                                    ...
                                    <textarea 
                                        ...
                                        rows="3"
                                        className="flex-1 bg-[#4E515E] border-none rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:ring-1 focus:ring-violet-500 outline-none transition-all text-sm shadow-inner resize-none"
                                    ></textarea>
                                </div>
                                    ...
                                    <>
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            <select 
                                                ...
                                                onChange={handleChange}
                                                className="flex-1 bg-[#4E515E] border-none rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-violet-500 outline-none transition-all text-sm shadow-inner"
                                            >
                        ...
                        >
                            ...
                            <h3 className="text-[#c4b5fd] font-bold text-base mb-8 tracking-wide">Attachment</h3>
                            
                            {file ? (
                                <div className="w-full flex flex-col items-center mb-6 animate-fade-in group mt-auto">
                                    <div className="h-14 w-14 bg-[#4E515E] rounded-2xl flex items-center justify-center mb-3 text-[#c4b5fd] shadow-md relative">
                                ...
                                <div className="w-full flex flex-col items-center mb-6 mt-auto">
                                     <div className="h-14 w-14 bg-[#4E515E] rounded-2xl flex items-center justify-center mb-3 text-[#c4b5fd] shadow-md">
                                ...
                                >
                                    <div className="bg-[#8b5cf6] text-white font-bold text-sm px-6 py-2.5 w-full rounded-sm shadow-sm hover:brightness-95 transition-all">
                    ...
                    <div className="mt-8">
                        <button 
                            ...
                            className={`px-8 py-2.5 rounded-xl text-sm font-bold tracking-wide shadow-md transition-all ${
                                submitting ? 'bg-[#8b5cf6]/50 text-white/50 cursor-not-allowed' : 'bg-[#8b5cf6] text-white hover:brightness-95'
                            }`}
                        >
