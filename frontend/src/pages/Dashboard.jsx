...
const Dashboard = () => {
    ...
    const { totals, vendorDistribution, campaignStats, dncStats, leadStatusBreakdown, recentSessions, refineCampaignStats, premiumCampaignStats, vanCampaignStats, separationCampaignStats, wcDbCampaignStats } = data;
        ...
        <div style={{ fontFamily:"'Inter',system-ui,sans-serif", paddingBottom:40 }}>
            ...
            <div style={{ marginBottom: 32 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:16 }}>
                    ...
                    <div style={{ maxWidth: '300px' }}>
                        <KpiCard index={1} icon={CheckCircle2} label="Total WC DB available" value={+(totals.total_wc_db_available || 0)} color="#10b981" sub="Ready to download" />
                    </div>
                    <div style={{ maxWidth: '300px' }}>
                        <KpiCard index={2} icon={Download} label="Total WC DB downloaded" value={+(totals.total_wc_db_downloaded || 0)} color="#f97316" sub="Already downloaded" />
                    </div>
                    {wcDbCampaignStats && wcDbCampaignStats.length > 0 && wcDbCampaignStats.map((camp, idx) => (
                        <div key={camp.name} style={{ maxWidth: '300px' }}>
                            <KpiCard
                                index={idx + 3}
                                icon={Target}
