    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-12 w-full h-[60vh] min-h-[400px]">
            <div className="relative flex items-center justify-center w-20 h-20 mb-6">
                <div className="absolute inset-0 bg-violet-500/20 rounded-full animate-ping blur-md"></div>
                <div className="absolute inset-2 bg-violet-400/30 rounded-full animate-pulse blur-sm"></div>
            ...
            <span className="font-extrabold tracking-widest uppercase text-sm text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-white animate-pulse">Syncing Session Details...</span>
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 text-slate-200 font-sans pb-12">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-[#1e1e2d] border border-white/10 rounded-2xl w-full max-w-xl relative z-10 overflow-hidden shadow-2xl">
                        ...
                        <div className="p-5 grid grid-cols-2 gap-3">
                            ...
                            {/* Fresh */}
                            <div className="bg-[#0a0a0f] rounded-xl border border-violet-500/20 p-4 relative overflow-hidden group hover:border-violet-500/40 transition-colors">
                                <div className="absolute top-0 left-0 w-1 h-full bg-violet-500"></div>
                                <div className="flex items-center gap-2 mb-2 ml-2">
                                    <TrendingUp className="w-3.5 h-3.5 text-violet-400" />
                                    <p className="text-violet-400 text-[11px] uppercase tracking-widest font-bold">Fresh Numbers</p>
                                </div>
            ...
            <div className="flex items-center space-x-2 text-[13px] text-slate-400 bg-[#1e1e2d] px-5 py-3.5 rounded-2xl shadow-sm border border-white/5 mx-auto">
                <Link to="/Van-sessions" className="text-violet-400 font-bold flex items-center cursor-pointer hover:text-violet-300 transition-colors">
            ...
            <div className="bg-[#1e1e2d] rounded-[2rem] p-8 sm:p-10 shadow-2xl border border-white/5 text-slate-200 relative overflow-hidden">
                {/* Decorative glow */}
                <div className="absolute -top-40 -right-40 w-96 h-96 bg-violet-500/10 rounded-full blur-[100px] pointer-events-none"></div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-white/10 pb-6 relative z-10 gap-4">
                    <h2 className="text-2xl font-extrabold tracking-tight flex items-center gap-3">
                        Active Jobs 
                        <span className="bg-[#0a0a0f] text-violet-400 px-3 py-1 rounded-lg text-lg border border-white/5 font-mono">
                            {session.jobs.length}
                        </span>
                    </h2>
                ...
                <div className="mb-10 relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        <div className="bg-[#0a0a0f] p-5 rounded-2xl border border-white/5 group hover:border-violet-500/30 transition-colors md:col-span-2">
                        ...
                        <div className="bg-[#0a0a0f] p-5 rounded-2xl border border-white/5 group hover:border-violet-500/30 transition-colors">
                            ...
                            <p className="text-[14px] font-medium text-white flex items-center truncate" title={session.created_by_username || 'System User'}>
                                <UserCircle className="w-4 h-4 mr-1.5 text-violet-400 shrink-0" /> {session.created_by_username || 'System User'}
                            </p>
                        </div>
                        <div className="bg-[#0a0a0f] p-5 rounded-2xl border border-white/5 group hover:border-violet-500/30 transition-colors md:col-span-2">
                        ...
                        <div className="bg-[#0a0a0f] p-5 rounded-2xl border border-white/5 group hover:border-violet-500/30 transition-colors">
                            ...
                            <p className="text-[14px] font-medium text-white flex items-center">
                                <span className="w-1.5 h-1.5 rounded-full bg-violet-400 mr-2"></span> Mixed Mode
                            </p>
                        </div>
                        <div className="bg-[#0a0a0f] p-5 rounded-2xl border border-white/5 group hover:border-violet-500/30 transition-colors">
                ...
                <div className="border-t border-white/10 pt-8 relative z-10">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        ...
                        <div className="flex space-x-3 w-full sm:w-auto">
                            <Link to={`/Van-sessions/${id}/add-job?bulk=true`} className="flex-1 sm:flex-none bg-[#0a0a0f] hover:bg-white/5 border border-white/10 text-slate-300 px-5 py-2.5 rounded-xl font-semibold transition-all flex items-center justify-center text-[13px] active:scale-[0.98]">
                                Bulk Upload
                            </Link>
                            <Link to={`/Van-sessions/${id}/add-job`} className="flex-1 sm:flex-none bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-500 hover:to-orange-400 text-white px-6 py-2.5 rounded-xl font-semibold transition-all shadow-[0_4px_14px_rgba(234,88,12,0.3)] flex items-center justify-center gap-2 active:scale-[0.98] text-[13px]">
                                                ...
                                                <button
                                                    ...
                                                    className={`flex items-center gap-2 text-left group ${
                                                        job.status === 'Completed'
                                                            ? 'cursor-pointer hover:text-violet-300 text-slate-200'
                                                            : 'cursor-default text-slate-400'
                                                    } transition-colors`}
                                                >
                                                    <FileText className={`w-3.5 h-3.5 shrink-0 ${
                                                        job.status === 'Completed' ? 'text-violet-400 group-hover:text-violet-300' : 'text-slate-600'
                                                    } transition-colors`} />
                                                    <span className="font-bold truncate max-w-[160px]">{job.file_name}</span>
                                                    {job.status === 'Completed' && (
                                                        <span className="text-[9px] bg-violet-500/15 border border-violet-500/25 text-violet-400 px-1.5 py-0.5 rounded font-bold shrink-0">STATS</span>
                                                    )}
                                                </button>
                                            </td>
                                                ...
                                                <div className="flex items-center gap-3 justify-center">
                                                    ...
                                                    <div className="w-16 h-1.5 bg-[#1e1e2d] rounded-full overflow-hidden border border-white/5">
                                                        <div className={`h-full rounded-full ${
                                                            job.status === 'Completed' ? 'bg-emerald-500' : job.status === 'Failed' ? 'bg-red-500' : 'bg-violet-500'
                                                        }`} style={{ width: job.status === 'Completed' ? '100%' : job.status === 'Failed' ? '0%' : '50%' }}></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4">
                                                <span className="text-slate-600 text-xs font-mono">â€”</span>
                                            </td>
...
export default VanSessionDetails;
