...
const RefineAddCampaign = ({ editMode = false }) => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchCampaign = async () => {
            try {
                const res = await api.get(`/refine-campaigns/${id}`);
                const data = res.data;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editMode) {
                await api.put(`/refine-campaigns/${id}`, data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                await api.post('/refine-campaigns', data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }

            navigate('/refine-campaigns');
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="min-h-screen font-sans" style={{ background: '#323644', margin: '-2rem', padding: '2rem' }}>
            ...
            <div className="flex items-center space-x-4 mb-8 text-white">
                <button 
                    onClick={() => navigate('/refine-campaigns')}
...
export default RefineAddCampaign;
