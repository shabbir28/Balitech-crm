...
const RefineDownloadLeads = () => {
    ...
    const pollScrubSummaryUntilDone = async (logId) => {
        ...
        try {
            while (Date.now() < deadline && !scrubPollCancelRef.current) {
                ...
                const statusRes = await api.get(`/refine-download/logs/${logId}/summary`);
    ...
    useEffect(() => {
        if (form.vendor_id) {
            const timeoutId = setTimeout(() => {
                ...
                api.post('/refine-download/state-counts', {
    ...
    useEffect(() => {
        if (form.vendor_id && form.vendor_id !== 'all') {
            const fetchFiles = () => {
                ...
                setSelectedFileId('');
                setVendorFiles([]);
                api.get(`/refine-vendors/${form.vendor_id}/files`)
                    .then(res => {
    ...
    useEffect(() => {
        if (selectedFileId) {
            const fetchStats = () => {
                setFileStats(null);
                api.get(`/refine-download/job/${selectedFileId}/stats`)
                    .then(res => {
    ...
    const handleDownloadCSV = async (req) => {
        setDlId(req.id);
        try {
            const res = await api.get(`/refine-download/requests/${req.id}/file`);
            setScrubSummaryData(res.data);
            if (res.data?.goodCsv) {
...
export default RefineDownloadLeads;
