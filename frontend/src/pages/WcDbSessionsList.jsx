...
const WcDbSessionsList = () => {
    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-6 font-sans pb-12 relative">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-[#1e1e2d] border border-white/10 rounded-2xl w-full max-w-md relative z-10 overflow-hidden shadow-2xl">
                        <div className="p-6">
                            ...
                            <p className="text-slate-400 text-[14px]">Are you sure you want to delete session <span className="font-mono text-white bg-white/5 px-2 py-0.5 rounded">{formatShortId(deleteModal.id)}</span>? <br /><span className="text-red-400 font-medium">All associated jobs will also be deleted.</span></p>
                        </div>
                        <div className="bg-black/20 p-4 border-t border-white/5 flex justify-end gap-3">
                            <button disabled={deleteModal.isDeleting} onClick={cancelDelete} className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-white hover:bg-white/5 transition-colors disabled:opacity-50">Cancel</button>
                            <button disabled={deleteModal.isDeleting} onClick={executeDelete} className="px-5 py-2 rounded-xl text-sm font-semibold bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 flex items-center gap-2 disabled:opacity-50">
            ...
            {filesModal.isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/70 backdrop-blur-md" onClick={() => { setFilesModal({ isOpen: false, files: [], jobsData: [], sessionId: null }); setSelectedJobStats(null); }} />
                    <div className="bg-[#1e1e2d] border border-white/10 rounded-2xl w-full max-w-xl relative z-10 overflow-hidden shadow-2xl">
                        <div className="flex items-center justify-between p-5 border-b border-white/8 bg-black/20">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 text-[11px] font-mono mt-0.5">{selectedJobStats ? <span className="truncate max-w-[260px] block">{selectedJobStats.file_name}</span> : `Session: ${formatShortId(filesModal.sessionId)}`}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                {!selectedJobStats && <span className="bg-cyan-500/15 border border-cyan-500/25 text-cyan-300 text-[11px] font-bold px-2.5 py-1 rounded-lg">{filesModal.files.length} {filesModal.files.length === 1 ? 'file' : 'files'}</span>}
                        ...
                        {!selectedJobStats ? (
                            ...
                            <div className="p-5 grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto">
                                ...
                                <div className="bg-[#0a0a0f] rounded-xl border border-orange-500/20 p-4 relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-1.5 mb-2 ml-2"><Copy className="w-3 h-3 text-orange-400" /><p className="text-orange-400 text-[10px] uppercase tracking-widest font-bold">Duplicates in File</p></div>
                        ...
                        <div className="px-4 py-3 border-t border-white/5 bg-black/20 flex justify-end">
                            <button onClick={() => { setFilesModal({ isOpen: false, files: [], jobsData: [], sessionId: null }); setSelectedJobStats(null); }} className="px-5 py-2 rounded-xl text-[13px] font-semibold text-slate-300 hover:text-white hover:bg-white/5">Close</button>
                        </div>
                    </div>
                </div>
            )}
