                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        ...
                        {(data.goodCsv || badCsv) && (
                            <button
                                type="button"
                                onClick={() => {
                                    const base = summary.fileName || `leads_${Date.now()}.csv`;
                                    const allName = base.replace(/\.csv$/i, '_all.csv');
                                    // Merge: skip header of badCsv if goodCsv present
                                    let merged = '';
                                    if (data.goodCsv && badCsv && String(badCsv).trim()) {
                                        const goodLines = String(data.goodCsv).trim().split('\n');
                                        const badLines = String(badCsv).trim().split('\n').slice(1); // skip bad header
                                        merged = [...goodLines, ...badLines].join('\n');
                                    } else {
                                        merged = data.goodCsv || badCsv || '';
                                    }
                                    downloadBlob(merged, allName);
                                }}
                                className="w-full flex items-center justify-center gap-2 py-3 bg-brand-500/10 hover:bg-brand-500/15 border border-brand-500/20 hover:border-brand-400/40 text-brand-400 font-bold text-xs rounded-xl transition-all"
                            >
                                <FileDown className="h-4 w-4 shrink-0" />
                                Download Full File (Good + Bad)&nbsp;&nbsp;<span className="bg-brand-500/20 px-2 py-0.5 rounded-full font-mono">{summary.total?.toLocaleString()}</span>
                            </button>
                        )}
                    </div>
                    ...
                    <div className="flex items-start gap-2 bg-blue-500/5 border border-blue-500/10 rounded-xl p-3 text-[11px] text-slate-400 leading-relaxed mt-1">
                        ...
                        <div>
                            ...
                            Flagged DNC leads have been saved to the local DNC database. You can still download them separately above.
                        </div>
                    </div>
                </div>
            )}
...
const PremiumDownloadLeads = () => {
    ...
    const pollScrubSummaryUntilDone = async (logId) => {
        ...
        try {
            while (Date.now() < deadline && !scrubPollCancelRef.current) {
                ...
                const statusRes = await api.get(`/premium-download/logs/${logId}/summary`);
    ...
    useEffect(() => {
        if (form.vendor_id) {
            const timeoutId = setTimeout(() => {
                ...
                api.post('/premium-download/state-counts', {
    ...
    useEffect(() => {
        if (form.vendor_id && form.vendor_id !== 'all') {
            const fetchFiles = () => {
                ...
                setSelectedFileId('');
                setVendorFiles([]);
                api.get(`/premium-vendors/${form.vendor_id}/files`)
                    .then(res => {
    ...
    useEffect(() => {
        if (selectedFileId) {
            const fetchStats = () => {
                setFileStats(null);
                api.get(`/premium-download/job/${selectedFileId}/stats`)
                    .then(res => {
    ...
    const fetchMyReqs = async () => {
        if (!isAdmin) return;
        setLoadingReq(true);
        try { const r = await api.get('/premium-download/requests/mine'); setMyRequests(r.data); }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isSuperAdmin) {
                ...
                const res = await api.post('/premium-download', body, { timeout: timeoutMs });
                setScrubSummaryData(res.data);
                // No auto-download — user picks Good / Bad / All from the summary panel
                ...
                await api.post('/premium-download/request', body);
    ...
    const handleDownloadCSV = async (req) => {
        setDlId(req.id);
        try {
            const res = await api.get(`/premium-download/requests/${req.id}/file`);
            setScrubSummaryData(res.data);
            if (res.data?.goodCsv) {
...
export default PremiumDownloadLeads;
