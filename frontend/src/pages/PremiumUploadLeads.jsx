...
const PremiumUploadLeads = () => {
    ...
    useEffect(() => {
        api.get('/premium-vendors?counts=false').then(res => setVendors(res.data)).catch(console.error);
        api.get('/premium-campaigns').then(res => {
    ...
    const handleCreateSession = async () => {
        ...
        try {
            ...
            const res = await api.post('/premium-sessions', {
                vendor_id: selectedVendor,
                campaign_type: campaignName
            });
            if (isCompareMode) {
                navigate(`/premium-sessions/${res.data.id}/add-job?mode=compare`);
            } else {
                navigate(`/premium-sessions/${res.data.id}`);
            }
        } catch (err) {
...
export default PremiumUploadLeads;
