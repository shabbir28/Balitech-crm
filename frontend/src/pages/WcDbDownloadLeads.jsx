...
const SelectInput = ({ value, onChange, disabled, required, children }) => (
    <div className="relative group">
        <select
            ...
            disabled={disabled}
            required={required}
            className="w-full bg-[#0a0c14]/50 backdrop-blur-md border border-white/10 text-white rounded-xl py-3.5 px-4 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500/60 transition-all cursor-pointer text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:border-cyan-500/30 shadow-inner group-hover:bg-[#0a0c14]/80"
        >
            {children}
        </select>
        <ChevronDown className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-hover:text-cyan-400 transition-colors" />
    </div>
);
    ...
    return (
        <div className="relative" ref={containerRef}>
            <button
                ...
                disabled={disabled}
                className={`w-full bg-[#0a0c14]/50 backdrop-blur-md border border-white/10 text-left rounded-xl py-3.5 px-4 flex justify-between items-center transition-all text-sm group shadow-inner ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#0a0c14]/80 hover:border-cyan-500/30'}`}
            >
                ...
                <ChevronDown className={`h-4 w-4 shrink-0 text-slate-500 transition-transform ${isOpen ? 'rotate-180 text-cyan-400' : 'group-hover:text-cyan-400'}`} />
            </button>
            {isOpen && (
                <div className="absolute z-50 w-full mt-1.5 bg-[#16192a] border border-white/10 rounded-xl shadow-2xl max-h-60 overflow-auto">
                    <div className="p-1.5 space-y-0.5">
                        {options.length === 0 ? (
                            ...
                            options.map(opt => (
                                <label key={opt.value} className="flex items-center gap-3 p-2.5 hover:bg-cyan-600/10 rounded-lg cursor-pointer transition-colors group">
                                    <input
                                        type="checkbox"
                                        checked={value.includes(opt.value)}
                                        onChange={() => toggleOption(opt.value)}
                                        className="h-4 w-4 rounded border-white/20 bg-black/20 text-cyan-500 focus:ring-cyan-500/30 focus:ring-offset-0 checked:border-cyan-500/50"
                                    />
...
const ScrubSummaryModal = ({ data, onClose }) => {
    ...
    const handleDownloadGood = () => {
        const fileData = data.goodCsv || data.csv;
        if (!fileData) return;
        downloadBlob(fileData, summary.fileName || `leads_${Date.now()}.csv`);
    };
            ...
            <div className="relative w-full max-w-5xl bg-[#13151f] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh] min-w-0">
                ...
                <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between bg-[#161824] w-full min-w-0">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-[#0a121a]merald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-950/40 p-4 border border-white/5 rounded-xl">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-sm font-semibold text-cyan-400 truncate" title={summary.fileName}>
                ...
                <div className="px-6 py-5 border-t border-white/[0.06] bg-[#161824] flex items-center justify-between flex-wrap gap-4 w-full min-w-0">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        {(data.goodCsv || data.csv) && (
                            <button
                                onClick={handleDownloadGood}
                                className="flex items-center gap-2 px-4 py-3 bg-emerald-500/10 hover:bg-emerald-500/15 border border-emerald-500/20 text-emerald-400 font-bold text-xs rounded-xl transition-all whitespace-nowrap"
                            >
                                <FileDown className="h-4 w-4 shrink-0" />
                                Download Good Leads ({summary.good?.toLocaleString()})
                            </button>
                        )}
        ...
        <div className="mt-6 bg-[#13151f]/80 backdrop-blur-xl border border-white/[0.07] rounded-2xl p-6 shadow-2xl relative animate-fade-in">
            ...
            <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-[#0a121a]merald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5 p-3 rounded-xl bg-[#0a0c14]/40 border border-white/5 text-[11px]">
                ...
                <div className="flex items-center gap-2 text-slate-400 min-w-0">
                    ...
                    <span className="font-semibold text-cyan-400 truncate" title={summary.fileName}>{summary.fileName || 'leads_scrubbed.csv'}</span>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
                ...
                {!isPending && (data.goodCsv || data.csv) && (
                    <button
                        type="button"
                        onClick={handleDownloadGood}
...
const WcDbDownloadLeads = () => {
    ...
    const pollScrubSummaryUntilDone = async (logId) => {
        ...
        try {
            while (Date.now() < deadline && !scrubPollCancelRef.current) {
                ...
                const statusRes = await api.get(`/wc-db-download/logs/${logId}/summary`);
    ...
    useEffect(() => {
        if (form.vendor_id) {
            const timeoutId = setTimeout(() => {
                ...
                api.post('/wc-db-download/state-counts', {
    ...
    useEffect(() => {
        if (form.vendor_id && form.vendor_id !== 'all') {
            const fetchFiles = () => {
                ...
                setSelectedFileIds([]);
                setVendorFiles([]);
                api.get(`/wc-db-vendors/${form.vendor_id}/files`)
                    .then(res => {
    ...
    useEffect(() => {
        if (selectedFileIds.length === 1) {
            const fetchStats = () => {
                setFileStats(null);
                api.get(`/wc-db-download/job/${selectedFileIds[0]}/stats`)
                    .then(res => {
    ...
    const fetchMyReqs = async () => {
        ...
        setLoadingReq(true);
        try { const r = await api.get('/wc-db-download/requests/mine'); setMyRequests(r.data); }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isSuperAdmin) {
                ...
                const res = await api.post('/wc-db-download', body, { timeout: timeoutMs });
    ...
    const handleDownloadCSV = async (req) => {
        setDlId(req.id);
        try {
            const res = await api.get(`/wc-db-download/requests/${req.id}/file`);
            setScrubSummaryData(res.data);
            if (res.data?.goodCsv) {
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-sm mt-1 font-medium max-w-lg leading-relaxed">
                                {isSuperAdmin
                                    ? 'Export WC DB leads with BLA compliance scrubbing.'
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
                    ...
                    <div className="bg-[#13151f]/70 backdrop-blur-2xl border border-white/[0.07] rounded-3xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.3)] relative">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            ...
                            <Field label="Vendor Source" required hint="Select the vendor whose data you want to export">
                                ...
                                {/* Vendor preview pill */}
                                {selectedVendor && (
                                    <div className="flex items-center gap-2 mt-2 px-3 py-2 bg-cyan-600/8 border border-cyan-500/50/15 rounded-lg w-fit">
                            ...
                            {form.vendor_id && form.vendor_id !== 'all' && (
                                <div className="relative z-20 p-4 rounded-2xl border border-white/[0.05] bg-white/[0.01] backdrop-blur-md space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        {loadingFiles && <span className="text-xs text-cyan-400 animate-pulse font-bold">Loading...</span>}
                                    </div>
                                ...
                                <Field label="State Filter" hint="Leave empty for all states">
                                    <div className="relative" ref={stateRef}>
                                        <button
                                            ...
                                            className="w-full bg-[#0a0c14]/50 backdrop-blur-md border border-white/10 hover:bg-[#0a0c14]/80 hover:border-cyan-500/30 text-white rounded-xl py-3.5 px-4 flex justify-between items-center transition-all text-sm group shadow-inner"
                                        >
                                            ...
                                            <ChevronDown className={`h-4 w-4 text-slate-500 group-hover:text-cyan-400 transition-transform ${stateOpen ? 'rotate-180 text-cyan-400' : ''}`} />
                                        </button>
                                        {stateOpen && (
                                            <div className="absolute z-50 w-full mt-1.5 bg-[#16192a] border border-white/10 rounded-xl shadow-2xl max-h-60 overflow-auto">
                                                <div className="p-1.5 space-y-0.5">
                                                    {US_STATES.map(s => (
                                                        <div
                                                            ...
                                                            className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors text-sm ${form.states.includes(s.abbr) ? 'bg-cyan-600/10 text-brand-300' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                                                        >
                                                            <div className={`h-4 w-4 rounded border flex items-center justify-center shrink-0 transition-all ${form.states.includes(s.abbr) ? 'bg-cyan-600 border-cyan-500/50' : 'border-slate-600'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <Field label="Min Age" hint="Optional">
                                        <input
                                            ...
                                            className="w-full bg-[#0a0c14]/50 backdrop-blur-md border border-white/10 hover:bg-[#0a0c14]/80 hover:border-cyan-500/30 focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/20 text-white rounded-xl py-3.5 px-4 outline-none transition-all text-sm font-mono shadow-inner"
                                            placeholder="25"
                                        />
                                    </Field>
                            ...
                            <Field label="Quantity" required hint="Max 100,000 per request recommended">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-[#0a0c14]/50 backdrop-blur-md border border-white/10 hover:bg-[#0a0c14]/80 hover:border-cyan-500/30 focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/20 text-white rounded-xl py-3.5 pl-11 pr-4 outline-none transition-all text-sm font-mono shadow-inner"
                                        placeholder="1000"
                                    />
                                </div>
                            </Field>
                                ...
                                className={`w-full flex items-center justify-center gap-3 py-4 rounded-xl font-bold text-sm transition-all duration-200 ${
                                    submitting || !form.vendor_id
                                        ...
                                        : isSuperAdmin
                                            ...
                                            : 'bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-400 hover:to-violet-500 text-white shadow-[0_8px_24px_rgba(59,130,246,0.3)] hover:shadow-[0_12px_32px_rgba(59,130,246,0.4)] hover:-translate-y-0.5'
                                }`}
                            >
                                {submitting ? (
                ...
                <div className="xl:col-span-2 space-y-4">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={s.step} className="flex items-start gap-3.5 group">
                                        <div className="h-7 w-7 rounded-xl bg-cyan-600/10 border border-cyan-500/50/20 text-cyan-400 text-xs font-black flex items-center justify-center shrink-0 group-hover:bg-cyan-600 group-hover:text-white transition-all shadow-sm shadow-brand-500/10">{s.step}</div>
                                        <div>
                    ...
                    <div className="bg-gradient-to-b from-[#161824] to-[#13151f] border border-white/[0.07] rounded-2xl p-6 shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-600/5 rounded-full blur-3xl pointer-events-none" />
                            ...
                            {form.vendor_id && (
                                <div className="mt-6 pt-5 border-t border-white/10">
                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
                                        ...
                                        <div className="flex items-center gap-3">
                                            {loadingCounts && <div className="h-3 w-3 border-2 border-cyan-500/30 border-t-brand-400 rounded-full animate-spin" />}
                                        ...
                                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2.5">
                                            {Object.entries(stateCounts)
                                                ...
                                                .map(([state, count]) => {
                                                    ...
                                                    return (
                                                        <div key={state} className="group relative bg-[#0a0c14]/60 border border-white/10 hover:border-cyan-500/50/40 rounded-xl p-2.5 overflow-hidden transition-all duration-300 shadow-sm hover:shadow-brand-500/20 hover:bg-[#0a0c14]/90 flex flex-col justify-between min-h-[60px]">
                                                            {/* Progress bar indicator at bottom */}
                                                            <div 
                                                                className="absolute bottom-0 left-0 h-[3px] bg-gradient-to-r from-cyan-600 to-violet-500 transition-all duration-1000 ease-out z-0 opacity-80" 
                                                                style={{ width: `${pct}%` }}
                                                            />
                ...
                <div className="mt-10">
                    ...
                    {loadingReq ? (
                        <div className="flex items-center justify-center py-16">
                            <div className="h-7 w-7 border-2 border-cyan-500/50/50 border-t-brand-400 rounded-full animate-spin" />
                        </div>
                    ) : myRequests.length === 0 ? (
                                                ...
                                                <td className="px-5 py-4">
                                                    {req.status === 'accepted' ? (
                                                        <button
                                                            ...
                                                            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#0a121a]merald-500/15 to-teal-500/15 hover:from-[#0a121a]merald-500/25 hover:to-teal-500/25 text-emerald-400 font-bold text-xs rounded-xl border border-emerald-500/25 transition-all hover:-translate-y-0.5"
                                                        >
                                                            {dlId === req.id
...
export default WcDbDownloadLeads;
