...
const Layout = ({ children }) => {
    ...
    return (
        <div className="min-h-screen flex" style={{ background: '#0f1117' }}>
            {/* Sidebar */}
            <div className="w-64 flex flex-col fixed min-h-screen z-10 border-r border-white/5" style={{ background: '#13151e' }}>
                {/* Logo */}
                ...
                {/* Nav */}
                <div className="flex-1 px-4 py-5 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="space-y-0.5">
                        {isAdmin && (
                            <>
                                ...
                                <NavLink to="/vendors" className={getClassName}>
                                    <Users className="h-5 w-5 flex-shrink-0" /> Vendor Panel
                                </NavLink>
                                <NavLink to="/upload" className={getClassName}>
                                    <UploadCloud className="h-5 w-5 flex-shrink-0" /> Upload Leads
                                </NavLink>
                                <NavLink to="/leads" className={getClassName}>
                                    <List className="h-5 w-5 flex-shrink-0" /> All Leads
                                </NavLink>
                        ...
                        <NavLink to="/download" className={getClassName}>
                            <Download className="h-5 w-5 flex-shrink-0" /> Download Leads
                        </NavLink>
                    </div>
                </div>
                ...
                <div className="p-4 border-t border-white/5">
                    <div className="flex items-center gap-3 px-2 mb-3">
                        <div className="h-9 w-9 rounded-full bg-amber-400 text-gray-900 flex items-center justify-center font-extrabold uppercase text-sm shadow-[0_0_10px_rgba(251,191,36,0.35)]">
                            {user?.username?.[0] || 'U'}
                        </div>
                        <div>
                    ...
                    <button 
                        ...
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-red-400 rounded-xl hover:bg-red-500/10 transition-colors"
                    >
