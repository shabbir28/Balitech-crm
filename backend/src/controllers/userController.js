...
const getUsers = async (req, res) => {
  try {
    ...
    if (search) {
      conditions.push(
        `(first_name ILIKE $${paramIdx} OR last_name ILIKE $${paramIdx} OR email ILIKE $${paramIdx} OR username ILIKE $${paramIdx})`,
      );
      params.push(`%${search}%`);
      paramIdx++;
    }
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Cannot modify a super admin account" });
    }
...
const deleteUser = async (req, res) => {
  const { id } = req.params;
  try {
    const existing = await db.query("SELECT role FROM users WHERE id = $1", [
      id,
    ]);
    if (existing.rows.length === 0)
      return res.status(404).json({ message: "User not found" });
    if (
      ...
      return res
        ...
        .json({
          message:
            "Only super admins can deactivate other super admin accounts",
        });
    }
