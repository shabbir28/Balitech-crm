...
const createCampaign = async (req, res) => {
  ...
  try {
    const result = await db.query(
      `INSERT INTO premium_campaigns (name, start_date, comments, attachment_url, attachment_name, status)
...
const getCampaigns = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === 'dialer_agent') {
        ...
        result = await db.query(
            `SELECT * FROM premium_campaigns ORDER BY created_at DESC`
        );
    }
    res.json(result.rows);
  } catch (err) {
    console.error("Error fetching premium_campaigns:", err);
    res.status(500).json({ message: "Server error fetching premium_campaigns" });
  }
};

// GET /api/premium_campaigns/:id
const getCampaignById = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await db.query(
      `SELECT * FROM premium_campaigns WHERE campaign_id = $1`,
      [id],
    );
...
const updateCampaign = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      attachment_url = `/uploads/premium_campaigns/${req.file.filename}`;
      attachment_name = req.file.originalname;

      // Get old file and delete it
      const old = await db.query(
        `SELECT attachment_url FROM premium_campaigns WHERE campaign_id = $1`,
        [id],
      );
...
// DELETE /api/premium_campaigns/:id
const deleteCampaign = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `DELETE FROM premium_campaigns WHERE campaign_id = $1 RETURNING *`,
      [id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "Campaign not found" });
