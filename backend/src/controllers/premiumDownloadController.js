...
const reviewDownloadRequest = async (req, res) => {
  ...
  try {
    ...
    if (goodRows.length === 0 && badRows.length === 0) {
      ...
      client.release();
      return res.status(404).json({
        message: "No available premium_data found. Request has been marked as rejected.",
      });
    }

    // Mark accepted immediately so admin sees status change
    await db.query(
      `UPDATE premium_download_requests SET status='accepted', reviewed_at=NOW(), reviewed_by=$1 WHERE id=$2`,
      [req.user.id, id],
    );
    ...
    res.json({
      message: `Request accepted. ${goodRows.length} premium leads are being prepared. The admin will be notified when ready to download.`,
    ...
    (async () => {
      ...
      try {
        ...
        await db.query(`UPDATE premium_download_requests SET csv_data=$1 WHERE id=$2`, [serializedData, id]);
        
        await db.query(
          `INSERT INTO premium_download_logs (user_id, vendor_id, campaign_id, quantity, states, min_age, max_age, csv_payload, download_date)
        ...
        await createNotification(
          ...
          `Your download request for ${finalGood.length.toLocaleString()} premium_data has been approved. You can now download your CSV file.`,
          dlReq.id,
        );
      } catch (bgErr) {
        console.error('[BG premium download] error:', bgErr.message);
        await bgClient.query('ROLLBACK').catch(() => {});
        await db.query(
          `UPDATE premium_download_requests SET status='rejected', rejection_reason=$1 WHERE id=$2`,
  ...
  client.release();
  console.error("Review Premium Download Request Error:", outerErr);
  if (!res.headersSent) {
    return res.status(500).json({ message: 'Server error processing request' });
  }
}
