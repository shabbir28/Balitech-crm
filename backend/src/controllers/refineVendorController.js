...
// POST /api/refine_vendors
const createVendor = async (req, res) => {
  const { name, company, email, phone, comment, status } = req.body;
  try {
    const result = await db.query(
      "INSERT INTO refine_vendors (name, company, email, phone, comment, status) VALUES ($1, $2, $3, $4, $5, COALESCE($6, 'Active')) RETURNING *",
...
const getVendors = async (req, res) => {
  ...
  try {
    ...
    console.error("Error fetching refine_vendors:", err);
    res.status(500).json({ message: "Server error" });
  }
};

// PUT /api/refine_vendors/:id
const updateVendor = async (req, res) => {
  const { id } = req.params;
  const { name, company, email, phone, comment, status } = req.body;
  try {
    const result = await db.query(
      `UPDATE refine_vendors 
...
// DELETE /api/refine_vendors/:id
const deleteVendor = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await db.query(
      "DELETE FROM refine_vendors WHERE vendor_id = $1 RETURNING *",
      [id],
    );
    if (result.rows.length === 0)
      return res.status(404).json({ message: "Vendor not found" });
...
// GET /api/refine_vendors/:id/files
