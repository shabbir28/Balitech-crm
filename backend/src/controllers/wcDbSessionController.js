...
const createSession = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      "INSERT INTO wc_db_sessions (vendor_id, campaign_type) VALUES ($1, $2) RETURNING *",
      [vendor_id, campaign_type || null]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.error("Error creating wc_db session:", err);
...
const getSessions = async (req, res) => {
  try {
    ...
    const countRes = await db.query(`
      ...
      FROM wc_db_sessions s
      LEFT JOIN wc_db_vendors v ON s.vendor_id = v.vendor_id
      ${whereClause}
    `, params);
    ...
    const result = await db.query(`
      ...
      FROM wc_db_sessions s
      LEFT JOIN wc_db_vendors v ON s.vendor_id = v.vendor_id
      LEFT JOIN wc_db_jobs j ON j.session_id = s.id
      ${whereClause}
    ...
    console.error("Error fetching wc_db_sessions:", err);
    res.status(500).json({ message: "Server error" });
  }
};
...
const getSession = async (req, res) => {
  ...
  try {
    ...
    const jobsRes = await db.query(
      "SELECT * FROM wc_db_jobs WHERE session_id=$1 ORDER BY created_at DESC",
      [id]
    );
    res.json({ ...sessionRes.rows[0], jobs: jobsRes.rows });
  } catch (err) {
    console.error("Error fetching wc_db session:", err);
    res.status(500).json({ message: "Server error" });
  }
};
...
const deleteSession = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await db.query("DELETE FROM wc_db_sessions WHERE id=$1 RETURNING *", [id]);
    if (result.rows.length === 0) return res.status(404).json({ message: "Session not found" });
    res.json({ message: "Session deleted successfully" });
  } catch (err) {
    console.error("Error deleting wc_db session:", err);
    res.status(500).json({ message: "Server error" });
  }
};
