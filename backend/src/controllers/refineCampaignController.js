...
const createCampaign = async (req, res) => {
  ...
  try {
    const result = await db.query(
      `INSERT INTO refine_campaigns (name, start_date, comments, attachment_url, attachment_name, status)
...
const getCampaigns = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Server error fetching refine_campaigns" });
  }
};

// GET /api/refine_campaigns/:id
const getCampaignById = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await db.query(
      `SELECT * FROM refine_campaigns WHERE campaign_id = $1`,
      [id],
    );
...
const updateCampaign = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      attachment_url = `/uploads/refine_campaigns/${req.file.filename}`;
      attachment_name = req.file.originalname;

      // Get old file and delete it
      const old = await db.query(
        `SELECT attachment_url FROM refine_campaigns WHERE campaign_id = $1`,
        [id],
      );
...
// DELETE /api/refine_campaigns/:id
const deleteCampaign = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `DELETE FROM refine_campaigns WHERE campaign_id = $1 RETURNING *`,
      [id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ message: "Campaign not found" });
