...
const createSession = async (req, res) => {
  try {
    ...
    const vendorCheck = await db.query(
      "SELECT vendor_id FROM refine_vendors WHERE vendor_id = $1",
      [vendor_id],
    );
    if (vendorCheck.rows.length === 0) {
      return res.status(404).json({ message: "Refine vendor not found" });
    }

    const campaignName = String(campaign_type).trim();
    const campaignCheck = await db.query(
      `SELECT name FROM refine_campaigns
       WHERE status = 'Active'
    ...
    if (campaignCheck.rows.length === 0) {
      return res.status(400).json({
        message:
          "Active refine campaign not found. Create the campaign under Refine Campaigns first.",
      });
    }

    const result = await db.query(
      `
            INSERT INTO refine_sessions (vendor_id, campaign_type, created_by)
            VALUES ($1, $2, $3)
            RETURNING *
        `,
    ...
    if (err.code === "23514") {
      return res.status(400).json({
        message:
          "Campaign type not allowed. Run migration 23_refine_sessions_campaign_flex.sql on the database.",
      });
    }
  ...
  try {
    ...
    const dataQuery = `
      WITH t AS (
        ...
        JOIN refine_vendors v ON s.vendor_id = v.vendor_id
        LEFT JOIN users u ON s.created_by = u.id
        LEFT JOIN refine_jobs j ON j.session_id = s.id
        ${where}
...
const deleteSession = async (req, res) => {
  try {
    ...
    const result = await db.query(
      "DELETE FROM refine_sessions WHERE id = $1 RETURNING id",
      [id],
    );
