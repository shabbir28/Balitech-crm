...
const createJob = async (req, res) => {
  try {
    ...
    if (validRecords.length === 0) {
      ...
      await db.query(
        `UPDATE upload_jobs SET status = 'Failed', error_message = 'No valid records found', end_time = CURRENT_TIMESTAMP WHERE id = $1`,
        [job.id],
      );
      return res
        .status(400)
    ...
    const recordsToInsert = uniqueRecords.filter((r) => !dncSet.has(r.phone) && !deadSet.has(r.phone) && !sepSet.has(r.phone));
...
const compareJob = async (req, res) => {
  try {
    ...
    const phonesNotDnc = uniquePhones.filter((p) => !dncSet.has(p) && !deadSet.has(p) && !sepSet.has(p));
