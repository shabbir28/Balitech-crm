...
const listDnc = async (req, res) => {
  try {
    ...
    const dataQuery = `
      ...
      FROM premium_dnc_numbers d
      LEFT JOIN users u ON d.created_by = u.id
      LEFT JOIN premium_campaigns c ON d.campaign_id = c.campaign_id
      ${where}
    ...
    const result = await db.query(
      `
        ...
        SET dnc_type = EXCLUDED.dnc_type,
            source = COALESCE(EXCLUDED.source, premium_dnc_numbers.source),
            notes = COALESCE(EXCLUDED.notes, premium_dnc_numbers.notes),
...
const importDnc = async (req, res) => {
  try {
    ...
    for (let i = 0; i < uniquePhones.length; i += BATCH) {
      ...
      const q = `
        INSERT INTO premium_dnc_numbers (phone, dnc_type, created_by, campaign_id)
...
const downloadDnc = async (req, res) => {
  try {
    ...
    const countResult = await db.query(
      `SELECT COUNT(*)::int AS count FROM premium_dnc_numbers d ${built.where}`,
      built.params,
    );
...
const deleteDnc = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await db.query(
      "DELETE FROM premium_dnc_numbers WHERE id = $1 RETURNING id",
      [id],
    );
    if (result.rows.length === 0) {
