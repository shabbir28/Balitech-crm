...
const createSession = async (req, res) => {
  try {
    ...
    const vendorCheck = await db.query(
      "SELECT vendor_id FROM premium_vendors WHERE vendor_id = $1",
      [vendor_id],
    );
    ...
    const result = await db.query(
      `
            INSERT INTO premium_sessions (vendor_id, campaign_type, created_by)
            VALUES ($1, $2, $3)
            RETURNING *
        `,
  ...
  try {
    ...
    const dataQuery = `
      WITH t AS (
        ...
        JOIN premium_vendors v ON s.vendor_id = v.vendor_id
        LEFT JOIN users u ON s.created_by = u.id
        LEFT JOIN premium_jobs j ON j.session_id = s.id
        ${where}
    ...
    res.status(500).json({ message: "Server error fetching premium_sessions" });
  }
};

// DELETE /api/premium_sessions/:id
const deleteSession = async (req, res) => {
  try {
    const { id } = req.params;

    const result = await db.query(
      "DELETE FROM premium_sessions WHERE id = $1 RETURNING id",
      [id],
    );
